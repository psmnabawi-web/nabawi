...
export async function POST(req: Request) {
  try {
    ...
    const now = Date.now();
    const store: Store = { id, code: body.code.toUpperCase(), name: body.name, city: body.city, active: body.active, createdAt: now, updatedAt: now };
    await ref.set(store);
...
export async function PATCH(req: Request) {
  try {
    ...
    const update = { code: body.code.toUpperCase(), name: body.name, city: body.city, active: body.active, updatedAt: Date.now() };
    ...
    await batch.commit();
    await writeAuditLog(ctx, { action: 'UPDATE_STORE', entity: 'store', entityId: body.id, details: update });
